...
def hello(path):
	global r, apis
	path = '/' + path
	if path == "/healthcheck":		#set this path for aws load balancer health check api
	...
	if ((path != '/api/v1/_health') or (path != '/api/v1/_crash')):
		r.tot_reqs += 1
	port = r.getnext()
	#print("http://3.210.166.12:"+str(port)+path)
...
def fun2():
	...
	while True:
		...
		for c, cont in enumerate(r.containers):
			...
			if requests.get("http://3.210.166.12:"+str(cont['port'])+"/api/v1/_health", headers=headers).status_code != 200:
				r.stopcontainer(c)
...
if  __name__ == "__main__":
	...
	thread3 = threading.Thread(target = fun2)
	thread1.start()
	thread2.start()
	thread3.start()
